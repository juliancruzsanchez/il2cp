...
let p = l => {
    if (file[l].match(/[0-9]+/g) == Array) return file[l].match(/[0-9]+/g)[0];else return file[l].match(/[0-9]+/g);
};

let b = l => {
    if (file[l].match(/[0-9]+/g)[0] === 0) return false;else return true;
};
...
function wing() {
    ...
    while (cl < a) {
        ...
        if (trim(file[a + 2]).startsWith('Skill0')) {
            ...
            if (trim(file[a + 2]).startsWith('Skill0')) output[currentLineInFile].skill.push(file[a + 2][9]);
            if (trim(file[a + 3]).startsWith('Skill1')) output[currentLineInFile].skill.push(file[a + 3][9]);
            if (trim(file[a + 4]).startsWith('Skill2')) output[currentLineInFile].skill.push(file[a + 4][9]);
            if (trim(file[a + 5]).startsWith('Skill3')) output[currentLineInFile].skill.push(file[a + 5][9]);
